...
public class SystemController {
    ...
    public Result<SystemRuleEntity> apiAdd(String app, String ip, Integer port,
        ...
        if (persistentApiClient instanceof MemoryApiClient) {
            ...
            if (!this.publishRules_persistence(app, ip, port)) {
                logger.warn("Publish system rules fail after rule add");
            }
        }
    ...
    private boolean publishRules_persistence(/*@NonNull*/ String app, String ip, Integer port) {
        ...
        boolean sentinelResult = sentinelApiClient.setSystemRuleOfMachine(app, ip, port, rules);
        boolean persistentResult = persistentApiClient.publishReturnBoolean(app, RuleConfigTypeEnum.SYSTEM, rules);
